...
% TODO(authors): replace with the final author list and affiliations.
\author{\IEEEauthorblockN{Author names withheld in draft}
\IEEEauthorblockA{\url{https://github.com/Ryclic/SpecHunter}}}
...
because the attack became irrelevant.
...
the exact source, binary, and runner that produced it. Our contributions are:
\begin{itemize}
...
\item End-to-end evidence on real BOOM RTL, clearly separated from fixture and
  model-only evidence (\S\ref{sec:eval}).
...
closed repair ID; it cannot emit patch text.
...
evidence file is bound into a SHA-256 seal that the test suite recomputes. A
locked JSON ledger reserves a conservative cost before each model call and
settles it from token metadata, halting at a hard cap. \sh{} runs as a
...
shown on a model fixture only. The search comparison uses three toy fixtures.
Repairs are chosen from a closed list, so the agent diagnoses rather
than synthesizes RTL, a deliberate trade of generality for soundness. The cache
observer covers two lines in a fixed order. Coverage-directed RTL
fuzzing~\cite{rfuzz} and speculation contracts~\cite{contracts} offer
complementary search and specification methods that could replace our fixed
operation set and invariant.
