...
A review against the sealed evidence found that the 2026-09-23 "submission package" section
below overstated results. Treat that section as superseded.
...
- Modules added on 2026-09-23 (`ablation`, `matrix`, `profiler`, `advisory`, `fpu`, `vector`,
  ...
  `contract`, `fuzzer`, `mcts`, `sva`, `synthesis`, `search`, `minimizer`, `redteam`,
  `differential`, `chisel_repair`, `chisel_testbench`, `poc`, `harness`, `taxonomy`,
  `waveform`, `autonomous_agent`) are not part of the evaluated loop. Several return fixed
  constants: FPU 17 cycles vs 0, ablation 68%/32%/89,200 sims/s, profiler IPC/LUT overheads,
  matrix "1091.7x", and `formal` choosing SAT/UNSAT from the `mitigated` flag without calling a
  solver. Each now carries an "ILLUSTRATIVE MODEL - NOT EVIDENCE" docstring banner, and its CLI
  command prints a warning. Deleting them was blocked by a permission check and is still the
  user's decision.
- The old paper (`paper/spechunter.tex`/`.typ`/`spechunter_micro2026.pdf`) cited those values. It
  also invented held-out corpus program names, said the corpus ran on a "patched" simulator
  (it ran on the pristine one, with a harness mutation), and said the LSU patch had "zero
  functional regression". Its bibliography had wrong authors and an unverifiable CHIA arXiv ID.
  The new paper is `paper/main.tex` + `paper/main.bib` → `paper/spechunter_a3_2026.pdf` (3
  pages, IEEEtran), built with `paper/build_main.sh`.
- `docs/demo.html` no longer shows the fabricated advisory (HSA-2026-0001/CVSS), the hand-drawn
  waveform (its cycle labels contradicted the witness), the benchmark chart, the taxonomy, or
  PoC panels. It is regenerated by `spechunter present`.
- `tools/verify_evidence.py` / `spechunter verify` (`spechunter.evidence_check`) recompute all 43
  file bindings across the 10 seals and rebuild the issue #715 case seal. CI runs this in place
  of the illustrative commands.
- `SUBMISSION.md` and the README header were rewritten to cover only sealed evidence, with
  limitations stated up front.
- Open: `chia_nodes.py` no longer matches the digest in `chia-vertex-loop-seal-2026-09-16.json`.
  Public `main` is still at PR #4 because PRs #5–#20 were stacked onto feature branches.
  The paper's author block is a placeholder.

Validation: `pytest -q -m 'not chia'` 445 passed, 1 skipped; Ruff clean;
`tools/verify_evidence.py` 0 failures; `tools/run_reproducibility_kit.sh` passes and
`docs/demo.html` matches a fresh render.
