...
- Modules added on 2026-09-23 were not part of the evaluated loop, and several returned fixed
  constants: FPU 17 vs 0 cycles, ablation 68%/32%/89,200 sims/s, profiler IPC/LUT overheads,
  matrix "1091.7x", and `formal` choosing SAT/UNSAT from the `mitigated` flag without calling a
  solver. With the user's approval, those 73 files (modules, tests, examples,
  benchmark/harness/packaging tools) were removed. `cli.py`, `chia_nodes.py`,
  `presentation.py`, `test_chia.py`, and `test_presentation.py` were restored to `d5179a5`, so
  `chia_nodes.py` again matches `chia-vertex-loop-seal-2026-09-16.json`.
...
- `tools/verify_evidence.py` (`spechunter.evidence_check`) recompute all 43
...
- Open: the paper's author block is a placeholder for the authors to fill in.
