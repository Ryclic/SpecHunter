...
patch_file="$script_directory/patches/issue_715_historical_gate_faulting_loads.patch"
echo "$BOOM_LOAD_GATE_PATCH_SHA256  $patch_file" | sha256sum -c -
git -C "$boom" apply "$patch_file"
echo "$BOOM_REPAIRED_LSU_SHA256  $boom/src/main/scala/lsu/lsu.scala" | sha256sum -c -
