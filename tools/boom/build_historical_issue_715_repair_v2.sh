...
patch_file="$script_directory/patches/issue_715_historical_block_speculative_loads.patch"
echo "$BOOM_REPAIR_V2_PATCH_SHA256  $patch_file" | sha256sum -c -
git -C "$boom" apply "$patch_file"
echo "$BOOM_REPAIR_V2_LSU_SHA256  $boom/src/main/scala/lsu/lsu.scala" | sha256sum -c -
