...
patch_file="$script_directory/patches/issue_715_historical_gate_spec_wakeup_on_dcache_fire.patch"
echo "$BOOM_REPAIR_V4_PATCH_SHA256  $patch_file" | sha256sum -c -
git -C "$boom" apply "$patch_file"
echo "$BOOM_REPAIR_V4_LSU_SHA256  $boom/src/main/scala/lsu/lsu.scala" | sha256sum -c -
